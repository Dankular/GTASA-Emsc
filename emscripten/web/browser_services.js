...
export class GameRuntimeController {
  constructor(module,input,draw=null){this.module=module;this.input=input;this.draw=draw;this.running=false;this.last=0;this.deviceLost=false;this.device=null;}
  ...
  frame(now){if(!this.running)return;const dt=Math.min((now-this.last)/1000,0.1);this.last=now;const s=this.input.pollGamepad();if(this.module._sa_runtime_tick_camera)this.module._sa_runtime_tick_camera(dt,s.steering,s.throttle,s.brake,s.cameraYaw,s.cameraPitch);else this.module._sa_runtime_tick(dt,s.steering,s.throttle,s.brake);this.draw?.({x:this.module._sa_runtime_vehicle_x(),y:this.module._sa_runtime_vehicle_y(),heading:this.module._sa_runtime_vehicle_heading(),speed:this.module._sa_runtime_vehicle_speed?.()||0,camera:this.module._sa_runtime_camera_x?{x:this.module._sa_runtime_camera_x(),y:this.module._sa_runtime_camera_y(),z:this.module._sa_runtime_camera_z(),lookX:this.module._sa_runtime_camera_look_x(),lookY:this.module._sa_runtime_camera_look_y(),lookZ:this.module._sa_runtime_camera_look_z()}:null,interior:this.module._sa_runtime_interior_active()===1,input:s});requestAnimationFrame(t=>this.frame(t));}
